...
> 오토인코더에서 학습데이터에 대해 원 데이터와 복원 데이터의 차이가 최대가 되는 지점을 Threshold(임계값)으로 설정하였다
...
> 실제 불량 데이터 7개에 대한 예측 값과 복원 데이터의 차이값을 히스토그램으로 보여주고 있다

- **딥러닝 모델과 FDC의 예측결과**
<img src="https://user-images.githubusercontent.com/121841464/232312505-7ab656df-3a7d-4f22-a378-211a972d1dc0.png" width="400">

> 현재 반도체 공장에서 사용하고 있는 FDC와 본연구의 딥러닝 모델의 예측결과, Precision은 약간 뒤쳐지지만 Accuracy, Recall, F1-score는 1~2% 의 개선된 결과를 보였다

----------
## 결론
본 연구에서는 반도체 박막 공정 데이터를 이용하여 제조제품 품질의 이상여부를 예측하기 위해 딥러닝을 사용한 예측모델을 개발하였다. 시계열 데이터에서 특징을 추출하여 데이터를 전처리하였고 추출된 값들을 사용하여 오토인코더 등 딥러닝 기반의 모델을 구축하고 성능을 분석하였으며, 오토인코더의 학습모델을 통해 원 데이터와 복원 데이터의 차이가 최대가 되는 지점을 임계값으로 설정하였다. 본 연구에 활용한 딥러닝 모델의 예측값이 현재 반도체 공장에서 사용하고 있는 FDC의 값보다 Accuracy, Recall, F1-score에 1~2% 우수한 결과를 보여주었다. 본 연구를 수행함에 있어서 미흡한 점은 실제 불량 데이터가 절대적으로 부족하여 이를 해결하기 위한 향후 연구가 필요하다.
